...
with DAG(start_date=datetime(2021, 1, 1), catchup=False, schedule_interval=None, dag_id="inference_dag") as dag:
    ...
    load_data = KubernetesPodOperator(
        name="load_data",
        image="anko47/classification-app:latest",
        cmds=["python", "classification/cli.py", "load-data", "/tmp/model/data/"],
        task_id="load_data",
        in_cluster=False,
        namespace="default",
        volumes=[volume],
    ...
    load_model = KubernetesPodOperator(
        name="load_model",
        image="anko47/classification-app:latest",
        cmds=["python", "classification/cli.py", "load-from-registry", "classification-model:latest",
              "/tmp/model/results/", "classification-losses"],
        task_id="load_model",
        env_vars={"WANDB_PROJECT": "classification-losses",
                  "WANDB_API_KEY": "***"},
        in_cluster=False,
        namespace="default",
        volumes=[volume],
