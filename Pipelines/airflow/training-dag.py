...
with DAG(start_date=datetime(2021, 1, 1), catchup=False, schedule_interval=None, dag_id="training_dag") as dag:
    ...
    load_data = KubernetesPodOperator(
        name="load_data",
        image="anko47/classification-app:latest",
        cmds=["python", "classification/cli.py", "load-mnist-data", "/tmp/data/"],
        task_id="load_data",
        in_cluster=False,
        namespace="default",
        volumes=[volume],
    ...
    train_model = KubernetesPodOperator(
        name="train_model",
        image="anko47/classification-app:latest",
        cmds=["python", "classification/cli.py", "train", "config/test_config.json"],
        task_id="train_model",
        in_cluster=False,
        namespace="default",
        volumes=[volume],
    ...
    upload_model = KubernetesPodOperator(
        name="upload_model",
        image="anko47/classification-app:latest",
        cmds=["python", "classification/cli.py", "upload-to-registry", "airflow-pipeline", "/tmp/results"],
        task_id="upload_model",
        env_vars={"WANDB_PROJECT": "classification_example", "WANDB_API_KEY": "***********"},
        in_cluster=False,
        namespace="default",
        volumes=[volume],
        volume_mounts=[volume_mount],
